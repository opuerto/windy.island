import { ProductDetailPagePropsInterface, MockDataResponseInterface } from '@/app/types/definitions';
import { getProduct } from '@/app/api/algoliaAPI/algoliaAPI';
...
export default async function PoductPage({ params }: ProductDetailPagePropsInterface) {
  ...
  const items: MockDataResponseInterface = await fetchProduct();

  const product = items.hits.map((item) => {
    return (
      <section key={item.id} className="text-gray-700 body-font overflow-hidden bg-white">
        <div className="container px-5 py-24 mx-auto">
          <div className="lg:w-4/5 mx-auto flex flex-wrap">
            <Image alt={item.name} width="500" height="500" className="lg:w-1/2 w-full object-cover object-center rounded border border-gray-200" src={item.image} />
